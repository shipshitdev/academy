...
export class MembershipsService {
  ...
  async create(createMembershipDto: CreateMembershipDto, userId: string): Promise<Membership> {
    const community = await this.communityModel.findById(createMembershipDto.communityId);
  ...
  async findMine(userId: string): Promise<Membership[]> {
    return this.membershipModel.find({ userId, status: 'active' }).sort({ createdAt: -1 });
  }
}
