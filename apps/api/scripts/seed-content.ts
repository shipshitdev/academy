...
import type {
  ICommunitySeed,
  ILessonSeed,
} from "./interfaces/seed-content.interface";
...
const COMMUNITY_SEEDS: ICommunitySeed[] = [
	{
		...
		slug: "vibe-coder",
		description:
			"Build fast, ship real software, and learn the indie workflow.",
		isFree: false,
		priceMonthly: 49,
		isFeatured: true,
		sortOrder: 1,
...
async function seed(): Promise<void> {
	const mongoUri =
		process.env.MONGODB_URI || "mongodb://localhost:27017/open-learning-center";
	...
	for (const communitySeed of COMMUNITY_SEEDS) {
		const community = await CommunityModel.findOneAndUpdate(
			{ slug: communitySeed.slug },
			{
				...
				priceMonthly: communitySeed.isFree
					? undefined
					: communitySeed.priceMonthly,
				isPublished: true,
